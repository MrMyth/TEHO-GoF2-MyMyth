...
//	objLandInterface.textinfo.datatext.scale = 0.9;
	...
	objLandInterface.imageslist.Altinfoback1.color = argb(128,64,64,64);
	objLandInterface.imageslist.Altinfoback1.uv = "0.0,0.0,1.0,0.125";
	objLandInterface.imageslist.Altinfoback1.pos = (sti(showWindow.left) + RecalculateHIcon(640 * fHtRatio)) + "," +
	RecalculateVIcon(12 * fHtRatio) + "," +
	...
	objLandInterface.textinfo.AltInfo4.pos.y = RecalculateVIcon(40 * fHtRatio);
	objLandInterface.textinfo.AltInfo4.refreshable = true;
	// быстрый переход
	if(CheckAttribute(loadedLocation,"fastreload")) // подсказка только там, где она нужна
	{
		objLandInterface.textinfo.AltInfo.font = "interface_normal"; //     сам текст вынесу отдельно
		objLandInterface.textinfo.AltInfo.color = argb(243,254,252,169);//  дабы не обновлЯть интерфейс полностью
...
void ModifyTextInfo() // belamour обновление всплывающей подсказки
{
	if(bGlobalVar1)
	{
		objLandInterface.textinfo.AltInfo3.text =  XI_convertString("ASBinfo"); 
		objLandInterface.textinfo.AltInfo4.text = XI_convertString("ASBcon"); 
		if(CheckAttribute(loadedLocation,"fastreload") && bFastEnable())
		{
			objLandInterface.textinfo.AltInfo.text  = XI_convertString("AFTinfo"); 
			objLandInterface.textinfo.AltInfo1.text = XI_convertString("AFTcon");
			objLandInterface.textinfo.AltInfo2.text = XI_convertString("AFTcon1");
		}
		else
		{
...
void BLI_SetPossibleCommands()
{
	...
	&& !LAi_grp_alarmactive && !chrDisableReloadToLocation && loadedLocation.type != "underwater") // belamour fix только не под водой
	{
...
void LI_ProcessControlPress()
{
	...
	switch(ControlName)
	{
		...
		case "LICommandsActivate": 
			...
			if(bGlobalVar1)
			{
				bGlobalVar1 = false; // belamour свернуть подсказку
				ModifyTextInfo();
			}				
		break;
	}
