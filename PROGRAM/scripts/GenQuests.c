...
void ShipWreck_DeliveToCity(string qName)
{
	...
	{
		chrDisableReloadToLocation = true;
		Log_Info("Shipwrecked sailors delivered");
...
void ShipWreck_MeetInShore(string qName)
{
	...
	for(int i = 0; i < sti(pchar.GenQuest.ShipWreck.Qty); i++)
	{
		...
		else // ура, место есть
		{
			...
			PlaceCharacter(sld, sLocGroup, "random_must_be"); // belamour gen: pchar.GenQuest.ShipWreck.Shore дает урезанный состав.  Или по задумке с голоду померли???
			if(i == 0)
			{
...
void ShipWreck_ShipSink(string qName)
{
	...
	{
		AddQuestUserData("ShipWrecked", "sText", "затонула, а с ней");
	}
	else
	{
		AddQuestUserData("ShipWrecked", "sText", "затонул, а с ним");
	}
	// <-- belamour
	CloseQuestHeader("ShipWrecked");
	DeleteAttribute(pchar, "GenQuest.ShipWreck");	
