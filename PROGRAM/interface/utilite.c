...
string GetItemDescribe(int iGoodIndex)
{
	...
	if (CheckAttribute(arItm, "groupID"))
	{
		...
		{
			if (HasSubStr(arItm.id, "mushket")) describeStr += GetAssembledString( LanguageConvertString(lngFileID,"mushket parameters"), arItm) + newStr();
			else 								describeStr += GetAssembledString( LanguageConvertString(lngFileID,"weapon gun parameters"), arItm) + newStr();
		}
