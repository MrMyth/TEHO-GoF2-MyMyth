...
void CS_TableSelectChange()
{
	...
	if (CurTable == "TABLE_SKILL_1" || CurTable == "TABLE_SKILL_2")
	{
		...
		{
	       GameInterface.TABLE_OTHER.tr8.td3.str = sti(GetSkillValueExp(xi_refCharacter, skillName))+ "/" + makeint(skillVal * GetCharacterExpRate(xi_refCharacter, skillName));
		}
	}
	Table_UpdateWindow("TABLE_OTHER");
...
void SetSkillArrows()
{
	...
	if (ok && CurTable == "TABLE_SPECIAL")
	{
        ...
        {
			SendMessage(&GameInterface,"lsllllll", MSG_INTERFACE_MSG_TO_NODE,"B_SKILLDown",0, 265 ,102 + 25*(iSelected-1), 297, 102 + 28 + 25*(iSelected-1), 0);
			SetNodeUsing("B_SKILLDown", true);
        }
        if (GetSkillValue(xi_refCharacter, SPECIAL_TYPE, GameInterface.(CurTable).(CurRow).UserData.ID) < SPECIAL_MAX)
        {
			SendMessage(&GameInterface,"lsllllll", MSG_INTERFACE_MSG_TO_NODE,"B_SKILLUP",0, 288,102 + 25*(iSelected-1),320,102 + 28 + 25*(iSelected-1), 0);
            SetNodeUsing("B_SKILLUP",  true);
        }
	}
}
...
void SetControlsTabMode(int nMode)
{
	...
	switch (nMode)
	{
		case 1: //
            ...
            GameInterface.TABLE_OTHER.tr8.td3.str = sti(xi_refCharacter.perks.FreePoints_self_exp)+ "/" + GetFreePoints_SelfRate(xi_refCharacter);
		break;
		case 2:
			sPic2 = "TabDeSelected";
            ...
            GameInterface.TABLE_OTHER.tr8.td3.str = sti(xi_refCharacter.perks.FreePoints_ship_exp)+ "/" + GetFreePoints_ShipRate(xi_refCharacter);
		break;
	}
...
void AcceptAddOfficer()
{
    ...
    if (checkAttribute(GameInterface, "PASSENGERSLIST."+attributeName2 + ".character"))
    {
    	...
    	if (!IsEntity(&worldMap) && !bSeaActive && bNeedFollow) // только для офов
    	{
			...
			else // Jason: залупу на воротник, а не обмен с офами, где их нет и быть не может. Супердыра закрыта.
			{
...
void FillPerksTable(string _type, bool _refresh)
{
	...
	for(i=0; i<perksQ; i++)
	{
		...
		if (CheckAttribute(arPerksRoot, perkName + ".BaseType") && arPerksRoot.(perkName).BaseType == _type)
		{
			...
			GameInterface.TABLE_PERKS.(row).td1.icon.width  = 29;
		    GameInterface.TABLE_PERKS.(row).td1.icon.height = 29;
		    GameInterface.TABLE_PERKS.(row).td1.icon.offset = "0, 0";
