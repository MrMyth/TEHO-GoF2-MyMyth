void QuestComplete(string sQuestName, string qname)
{
	...
	{	
		...
		case "pchar_fast_mushket":
			...
			{
				Lai_SetPlayerType(pchar);
				SetMainCharacterToMushketer(pchar.GenQuest.MusId, true);
				PlaySound("PEOPLE FIGHT\Player_Man_02_02.wav");
			}
			else
			{
				Lai_SetPlayerType(pchar);
				SetMainCharacterToMushketer("", false);
				EquipCharacterbyItem(pchar, pchar.GenQuest.MusId);
				PlaySound("PEOPLE FIGHT\Blade_Take_In_0"+(rand(2)+1) +".wav");
			}
			DeleteAttribute(pchar, "GenQuest.MusId");
		break;
...
/// Jason ----------------------------------------------------------- Дороже золота ------------------------------------------------------------------
		...
		case "GoldenGirl_ReloadToBaster": //
			...
			SetLaunchFrameFormParam("A few days have passed..."+ NewStr() +"You and baronet Courtney arrived to Guadeloupe", "", 0, 5);
			WaitDate("", 0, 0, 3, 2, 10); 
			LaunchFrameForm();
			RefreshLandTime();
			RecalculateJumpTable();
