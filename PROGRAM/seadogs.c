...
void ProcessControls()
{
	...
	else
	{
		switch(ControlName)
		{
			...
			case "MainMenuDouble": ProcessMainMenuKey(); break;
			case "Interface": 
			if(loadedLocation.type == "underwater") return; // belamour фикс прогулок под водой
			ProcessInterfaceKey();
			break;
			case "Tele": 
				StartQuickTeleport();
...
//			}
		...
		case "AltModificatorOff":
			...
			PlaySound("interface\minus_001.wav");
			bGlobalVar1 = false;
			ModifyTextInfo();
		break;
		...
		case "AltModificator":
			if (!IsEntity(&ILogAndActions)) return;
			if(loadedLocation.type == "Underwater") return;
			PlaySound("interface\plus_001.wav");
			bGlobalVar1 = true;
		...
		case "ChrFightMode":
			if(bGlobalVar1)
			{
				bGlobalVar1 = false; // свернуть подсказку
				ModifyTextInfo();	
			}				
		break;
		...
		case "Dolly":
			...
			if (CheckAttribute(location, "id") && location.id == "Treasure_Alcove" && !bGlobalVar1) // калеуче
			{
...
*/			
        ...
        case "BOAL_ActivateRush":  // boal KEY_F
            ...
            {
                if (bLandInterfaceStart && curKeyGroupName == "FightModeControls" && !bGlobalVar1)
                {
