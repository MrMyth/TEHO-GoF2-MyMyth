...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
    ...
    switch (Dialog.CurrentNode)
	{
		...
		if(sti(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			dialog.text = "Let's register: captain "+ GetFullName(pchar) +", ship is "+pchar.Ship.Name+"... Regatta's time in hours is "+sti(pchar.questTemp.Regata.SecondTransitionTime)+". Done, your result is registered, you can continue your way.";
